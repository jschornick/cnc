...
void tmc_init(void)
{
  ...
  tmc_config_t config;
  config.drvconf = drvconf_init;
  config.drvctl = drvctl_init;
  config.chopconf = chopconf_init;
  config.sgcsconf = sgcsconf_init;
  config.smarten = smarten_init;
...
void tmc_reconfigure(uint8_t tmc, tmc_config_t *config)
{
  ...
  tmc_send(tmc, tmc_config[tmc].drvconf);
  tmc_send(tmc, tmc_config[tmc].drvctl);
  tmc_send(tmc, tmc_config[tmc].chopconf);
  tmc_send(tmc, tmc_config[tmc].sgcsconf);
  tmc_send(tmc, tmc_config[tmc].smarten);
}
