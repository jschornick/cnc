...
void decode_response(uint32_t response)
{
  uart_queue_str("TMC response:\r\n");
  uart_queue_str("-------------\r\n");
  uart_queue_str("(00) StallGuard status   : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) (response&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(01) Over temp shutdown  : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>1)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(02) Over temp warning   : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>2)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(03) Short to ground (A) : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>3)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(04) Short to ground (B) : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>4)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(05) Open load (A)       : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>5)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(06) Open load (B)       : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>6)&0x01)));
  uart_queue_str("\r\n");
  uart_queue_str("(07) Standstill          : ");
  uart_queue(NIBBLE_TO_ASCII((uint8_t) ((response>>7)&0x01)));
  uart_queue_str("\r\n");

  switch(RDSEL) {
    case RD_USTEP:
      uart_queue_str("     MSTEP               : ");
      uart_queue_hex(response>>10, 9);
      uart_queue_str("  (POL: ");
      uart_queue_hex(response>>19, 1);
      uart_queue_str(")\r\n");
      break;
    case RD_SG:
      uart_queue_str("     SG2                 : ");
      uart_queue_hex(response>>10, 10);
      uart_queue_str("\r\n");
      break;
    case RD_SG_CS:
      uart_queue_str("     SG2                 : ");
      uart_queue_hex(response>>15, 5);
      uart_queue_str("\r\n");
      uart_queue_str("     CS                  : ");
      uart_queue_hex(response>>10, 5);
      uart_queue_str("\r\n");
      break;
  }
}
...
int main(void) {
  ...
  // [ 3.. 0] =1000  : Off time
  spi_rx = spi1_send(0x901B4);  // hysteresis mode
  decode_response(spi_rx);

  spi_rx = spi1_send(0xE0000); // step/dir mode, read microstep
  decode_response(spi_rx);
  ...
  while(1) {
    if(S1_FLAG) {
      ...
      spi_rx = spi1_send(0xE0000); // low driver strength, read microstep pos, step mode
      uart_queue_str("\r\n");
      decode_response(spi_rx);
      RDSEL = RD_USTEP;
      S1_FLAG=0;
    ...
    if(S2_FLAG) {
      /* uart_queue_str("\r\nB2: Reading SG2/CS\r\n"); */
      /* //spi_rx = spi1_send(0xE0010); // low driver strength, read stallGuard, step mode */
      /* spi_rx = spi1_send(0xE0020); // low driver strength, read stallGuard, step mode */
      /* uart_queue_str("\r\n"); */
      /* decode_response(spi_rx); */
      /* RDSEL = RD_SG_CS; */
      uart_queue_str("\r\nB2: Step\r\n");
